import {editHiringRecordUsingPost} from '@/services/backend/hiringRecordController';
import {uploadFileUsingPost} from '@/services/backend/fileController';
import {checkFile,} from '@/utils';
import {
  ProForm, ProFormDateTimePicker, ProFormDependency,
...
interface Props {
  visible: boolean;
  oldData: API.HiringRecordVO | undefined;
...
const UpdateModal: React.FC<Props> = (props) => {
   ...
   * @param fields
   */
  const handleUpdate = async (fields: API.HiringRecordEditRequest) => {
    const hide = message.loading('正在修改');
    try {
      await editHiringRecordUsingPost(fields);
      hide();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ProFormSelect label={"当前状态"} name={"status"} options={statusOptions}
                       rules={[{required: true, message: '状态不能为空'}]}/>
        <ProFormDependency name={['status']}>
          {({ status }) => {
            if (status === 1) {
              return (
                <ProFormDateTimePicker  name="leaveDate" label="离职时间" rules={[{ required: true, message: '离职时间不能为空' }]} />
              );
            }
            return null;
          }}
